...
# ===================================================
# Read and analyze decomposition_joined.csv
# from PPML baseline regression outputs
# ===================================================
...
REG_RESULTS <- file.path(OUT_DIR, "regression_decomposition_results.csv")
COVAR_RESULTS <- file.path(OUT_DIR, "covariate_decomposition_results.csv")
...
city_summary <- decomp %>%
  group_by(first_city) %>%
  summarise(
    mean_fe_city  = mean(fe_city, na.rm = TRUE),
    mean_fe_firm  = mean(fe_firm, na.rm = TRUE),
    mean_fe_user  = mean(fe_user, na.rm = TRUE),
    n_obs = n()
  ) %>%
  arrange(desc(n_obs))
...
write_csv(city_summary, file.path(OUT_DIR, "city_decomposition_summary.csv"))
write_csv(firm_summary, file.path(OUT_DIR, "firm_decomposition_summary.csv"))

cat("[INFO] Done. Results saved to:\n")
cat("  -", file.path(OUT_DIR, "city_decomposition_summary.csv"), "\n")
cat("  -", file.path(OUT_DIR, "firm_decomposition_summary.csv"), "\n")
...
fe_data <- decomp %>%
  select(fe_user, fe_firm, fe_city) %>%
  na.omit()
...
write_csv(as.data.frame(cov_matrix), file.path(OUT_DIR, "fe_covariance_matrix.csv"))
write_csv(as.data.frame(cor_matrix), file.path(OUT_DIR, "fe_correlation_matrix.csv"))

cat("[INFO] Saved covariance/correlation results to:\n")
cat("  -", file.path(OUT_DIR, "fe_covariance_matrix.csv"), "\n")
cat("  -", file.path(OUT_DIR, "fe_correlation_matrix.csv"), "\n")

...
##############################################################################
# ppml_tenure_ladder_seniority.R
#
# This script estimates seniority-valid PPML lifecycle patenting profiles by
# tenure bin using inventor-year panel data from:
#   /labs/khanna/linkedin_202507/processed/inventor_year_merged_v2
#
# The script:
#   1. Loads inventor-year data for 2000--2024 and restricts to U.S.
#      inventor-years.
#   2. Constructs tenure, tenure bins, reference year, parent-firm, metro,
#      metro-parent, and seniority identifiers.
#   3. Constructs seniority_bin from avg_seniority when available, rounded to
#      categorical values 1--7, and saves seniority missingness/rounding
#      diagnostics.
#   4. Constructs:
#        - parent_seniority_fe = parent_fe_id x seniority_bin
#        - mp_seniority_fe     = mp_fe_id x seniority_bin
#   5. Reads the final Revelio-to-CBSA density crosswalk and merges first-metro
#      population, land area, and density to inventor-level first metro.
#   6. Constructs four top/complement splits:
#        - CAREER_TOPCOMP
#        - FIRSTMETRO_SIZE_TOPCOMP
#        - FIRSTMETRO_PROD_TOPCOMP
#        - FIRSTMETRO_DENSITY_TOPCOMP
#   7. Constructs inventor-level mobility samples:
#        - full_sample
#        - same_start_end_metro
#        - never_moved_metro
#        - same_start_end_mp
#        - never_moved_mp
#   8. Estimates PPML lifecycle ladders on the seniority-valid sample for:
...
#          interactions.
#   9. Uses a decomposition-oriented seniority-valid FE ladder:
...
#        - plus_parent_seniority_fe:        user_id + year_fe + parent_seniority_fe
#        - plus_mp_parent_seniority_fe:     user_id + year_fe + mp_fe + parent_seniority_fe
#  10. Clusters standard errors at the metro-parent level.
#  11. Ranks the top 10 firms by total patent counts, not average patenting,
#      within each analysis window, then saves firm-by-seniority and pooled
#      top-10-firm seniority diagnostics and plots.
#  12. Saves raw coefficients, standard errors, confidence intervals, reference
#      bin means, implied level-scale plotting variables, log-coefficient plots,
#      support diagnostics, lifecycle-growth decomposition tables, plots, and
#      manifests.
#  13. Does not run cohort-specific analyses. If RUN_COHORTS is exported by
#      sbatch, it is ignored harmlessly.
#
# Output directory:
#   /home/epiga/revelio_labs/output/ppml_tenure_ladder_seniority_revised
#
# Plotting convention:
#   PPML estimates are log-link coefficients. Level-scale lifecycle plots are
#   converted to implied levels using:
#       plot_estimate_b = mean_patents_0_5_group * exp(beta_b)
#   with beta_0-5 = 0. Log-coefficient lifecycle plots are also saved.
##############################################################################
...
OUT_DIR <- "/home/epiga/revelio_labs/output/ppml_tenure_ladder_seniority_revised"
...
SENIORITY_CANDIDATES <- c("avg_seniority", "mean_seniority", "seniority")
OPTIONAL_SENIORITY_COLS <- c("avg_seniority", "min_seniority", "max_seniority")
...
prepare_analysis_sample <- function(dd0) {
  dd0 %>%
    filter(
      ...
      !is.na(seniority_bin),
      !is.na(parent_seniority_fe_id), parent_seniority_fe_id != "",
      !is.na(mp_seniority_fe_id), mp_seniority_fe_id != ""
    ) %>%
    mutate(
      tenure_bin = make_tenure_bins(tenure),
      tenure_bin = factor(as.character(tenure_bin), levels = ordered_tenure_levels(), ordered = TRUE),
      seniority_bin = factor(as.character(seniority_bin), levels = as.character(1:7), ordered = TRUE),
      user_id    = as.factor(user_id),
      year_fe    = as.factor(year),
      parent_fe  = as.factor(parent_fe_id),
      metro_fe   = as.factor(metro_fe_id),
      mp_fe      = as.factor(mp_fe_id),
      parent_seniority_fe = as.factor(parent_seniority_fe_id),
      mp_seniority_fe = as.factor(mp_seniority_fe_id),
...
ALLOWED_SPECS_WITH_USER_FE <- c(
  ...
  "plus_metro_fe",
  "plus_mp_fe",
  "plus_parent_seniority_fe",
  "plus_mp_parent_seniority_fe"
)
...
fe_ladder_user <- list(
  ...
  list(spec_name = "plus_parent_seniority_fe",        fe = "user_id + year_fe + parent_seniority_fe"),
  list(spec_name = "plus_mp_parent_seniority_fe",     fe = "user_id + year_fe + mp_fe + parent_seniority_fe")
)
...
make_support_diagnostics <- function(dd, analysis_name, split_var, window_tag, sample_tag) {
  ...
  diag <- out %>%
    ...
    summarise(
      ...
      mean_seniority = mean(as.numeric(as.character(seniority_bin)), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(
    ...
    select(analysis, window, sample_tag, cohort_tag, diag_group, tenure_bin,
           n_obs, n_users, n_clusters, n_positive, mean_patents, mean_seniority,
...
run_full_unsplit_ppml <- function(df_base, window_tag, sample_tag, fe_ladder) {
  ...
  ts_msg("Starting full unsplit seniority-valid lifecycle block:",
         "window=", window_tag,
         "| sample=", sample_tag)
  ...
  ts_msg("Full unsplit seniority-bin counts:")
  print(dd0 %>% count(seniority_bin, name = "n_rows") %>% arrange(seniority_bin))
...
run_separate_one_group_ppml <- function(dd0, split_var, group_value, profile_name,
  ...
  ts_msg("Prepared separate-group seniority-valid sample:",
         "analysis=", analysis_name,
  ...
  ts_msg("Seniority-bin counts for separate-group sample:")
  print(dd_g %>% count(seniority_bin, name = "n") %>% arrange(seniority_bin))
...
run_pooled_topcomp_ppml <- function(dd0, split_var, analysis_name, window_tag,
  ...
  ts_msg("Prepared pooled saturated seniority-valid sample:",
         "analysis=", analysis_name,
         "| rows=", nrow(dd_i),
  ...
  ts_msg("Group-by-seniority counts for pooled sample:")
  print(dd_i %>% count(group2, seniority_bin, name = "n") %>% arrange(group2, seniority_bin))
...
run_topcomp_block <- function(df_base, split_var, analysis_name, window_tag,
  ...
  ts_msg("Starting seniority-valid top/complement lifecycle block:",
         "window=", window_tag,
         "| analysis=", analysis_name,
         "| split_var=", split_var,
         "| sample=", sample_tag)
  ...
  ts_msg("Group-by-seniority counts:")
  print(
    dd0 %>%
      group_by(group = .data[[split_var]], seniority_bin) %>%
      summarise(n_rows = n(), n_users = n_distinct(user_id),
                n_positive = sum(n_patents > 0, na.rm = TRUE), .groups = "drop") %>%
      arrange(group, seniority_bin)
  )
...
plot_profile_core <- function(tab, window_tag, analysis_name, profile_source,
  ...
  p <- ggplot(tab, aes(x = tenure_bin, y = plot_estimate, color = spec_name, group = spec_name)) +
    ...
    labs(
      title = paste0(title_stub, " ", profile, " lifecycle profile, seniority-valid — ", analysis_name, " — ", window_tag),
      subtitle = paste0("sample=", sample_tag, " | all cohorts | seniority-valid sample"),
      x = "Tenure bin",
      y = ylab,
      color = "Specification",
...
run_seniority_descriptives <- function(df_window, window_tag) {
  ts_msg("Running top-10 total-patent firm seniority descriptives for window:", window_tag)
  ...
  if (nrow(dd) == 0) {
    ts_msg("No rows available for seniority descriptives.")
    return(invisible(NULL))
  }

  firm_rank <- dd %>%
    group_by(parent_fe_id) %>%
    summarise(
    ...
    arrange(desc(total_patents), desc(n_users), desc(n_inventor_years)) %>%
    slice_head(n = 10)

  ts_msg("Top 10 firm IDs by total patents:")
  print(firm_rank)

  out_rank <- file.path(dirs$diagnostics, paste0(window_tag, "__top10_firms_by_total_patents.csv"))
  write_csv(firm_rank, out_rank)
  ts_msg("Saved top-firm diagnostic:", out_rank)

  top_ids <- firm_rank$parent_fe_id

  firm_seniority <- dd %>%
    filter(parent_fe_id %in% top_ids) %>%
    group_by(parent_fe_id, seniority_bin) %>%
    summarise(
      total_patents = sum(n_patents, na.rm = TRUE),
      mean_patents = mean(n_patents, na.rm = TRUE),
      n_obs = n(),
    ...
    left_join(firm_rank %>% select(parent_fe_id, firm_total_patents = total_patents), by = "parent_fe_id") %>%
    arrange(desc(firm_total_patents), parent_fe_id, seniority_bin)
  ...
  pooled_seniority <- dd %>%
    ...
    arrange(seniority_bin)

  ts_msg("Firm x seniority diagnostics:")
  print(firm_seniority)

  ts_msg("Pooled top-10-firm seniority diagnostics:")
  print(pooled_seniority)

  out_firm_sen <- file.path(dirs$diagnostics, paste0(window_tag, "__top10_total_patent_firms_by_seniority_counts.csv"))
  out_pool_sen <- file.path(dirs$diagnostics, paste0(window_tag, "__top10_total_patent_firms_pooled_by_seniority_counts.csv"))

  write_csv(firm_seniority, out_firm_sen)
  write_csv(pooled_seniority, out_pool_sen)

  ts_msg("Saved firm x seniority counts:", out_firm_sen)
  ts_msg("Saved pooled seniority counts:", out_pool_sen)

  p1 <- ggplot(firm_seniority, aes(x = seniority_bin, y = mean_patents, group = parent_fe_id, color = parent_fe_id)) +
    geom_line(linewidth = 0.8, alpha = 0.9) +
    geom_point(size = 2.4) +
    labs(
      title = paste0("Average patents by seniority bin for top 10 total-patent firms — ", window_tag),
      subtitle = "Top firms ranked by total patents in the analysis window",
      x = "Seniority bin",
      y = "Average patents per inventor-year",
      color = "Parent firm ID"
    ) +
    theme_minimal(base_size = 12) +
  ...
  save_plot(
    p1,
    file.path(dirs$plots, paste0(window_tag, "__top10_total_patent_firms_patents_by_seniority.png")),
    width = 12,
    height = 7
  )

  p2 <- ggplot(pooled_seniority, aes(x = seniority_bin, y = mean_patents, group = 1)) +
    geom_line(linewidth = 0.8) +
    geom_point(size = 2.8) +
    labs(
      title = paste0("Average patents by seniority bin, pooled top 10 total-patent firms — ", window_tag),
      x = "Seniority bin",
      y = "Average patents per inventor-year"
    ) +
    theme_minimal(base_size = 12)
  ...
  save_plot(
    p2,
    file.path(dirs$plots, paste0(window_tag, "__top10_total_patent_firms_pooled_patents_by_seniority.png")),
    width = 9,
    height = 6
  )
...
save_growth_decomposition <- function(profile_all, tag = "chunk") {
  ...
  growth <- profile_all %>%
    ...
    mutate(
      ...
      parent_growth = lifecycle_growth[spec_name == "plus_parent_fe"][1],
      mp_growth = lifecycle_growth[spec_name == "plus_mp_fe"][1],
      incremental_seniority_vs_parent =
        ifelse(spec_name == "plus_parent_seniority_fe",
               1 - lifecycle_growth / parent_growth,
               NA_real_),
      incremental_seniority_vs_mp =
        ifelse(spec_name == "plus_mp_parent_seniority_fe",
               1 - lifecycle_growth / mp_growth,
               NA_real_)
    ) %>%
    ungroup()

  out <- file.path(dirs$tables, paste0(tag, "__seniority_valid_lifecycle_growth_decomposition.csv"))
  write_csv(growth, out)
  ts_msg("Saved seniority-valid lifecycle-growth decomposition:", out)
}
...
SENIORITY_VAR <- pick_first_existing(SENIORITY_CANDIDATES, ds_names, what = "seniority")

ts_msg("Using metro variable:", METRO_VAR)
ts_msg("Using gender variable:", GENDER_VAR)
ts_msg("Using seniority variable:", SENIORITY_VAR)
...
need_cols(ds_names, c(
  ...
  PARENT_VAR, METRO_VAR, GENDER_VAR, SENIORITY_VAR
))

seniority_optional_present <- intersect(OPTIONAL_SENIORITY_COLS, ds_names)
...
cols_pull <- unique(c(
  ...
  "last_university_country", FIRST_CITY_VAR,
  PARENT_VAR, METRO_VAR, GENDER_VAR, SENIORITY_VAR,
  seniority_optional_present
))

ts_msg("Collecting seniority data:", CHAR_FULL_START, "to", CHAR_FULL_END)
...
if (!("min_seniority" %in% names(df_full))) df_full$min_seniority <- NA_real_
if (!("max_seniority" %in% names(df_full))) df_full$max_seniority <- NA_real_
...
df_full <- df_full %>%
  mutate(
    ...
    seniority_raw = suppressWarnings(as.numeric(.data[[SENIORITY_VAR]])),
    min_seniority_raw = suppressWarnings(as.numeric(min_seniority)),
    max_seniority_raw = suppressWarnings(as.numeric(max_seniority))
  ) %>%
  mutate(
  ...
  mutate(
    ...
    seniority_rounded = round(seniority_raw),
    seniority_round_abs_change = abs(seniority_raw - seniority_rounded),
    seniority_bin = case_when(
      is.finite(seniority_rounded) & seniority_rounded >= 1 & seniority_rounded <= 7 ~ as.integer(seniority_rounded),
      TRUE ~ NA_integer_
    ),
    parent_seniority_fe_id = case_when(
      !is.na(parent_fe_id) & parent_fe_id != "" & !is.na(seniority_bin) ~ paste0(parent_fe_id, "__seniority_", seniority_bin),
      TRUE ~ NA_character_
    ),
    mp_seniority_fe_id = case_when(
      !is.na(mp_fe_id) & mp_fe_id != "" & !is.na(seniority_bin) ~ paste0(mp_fe_id, "__seniority_", seniority_bin),
      TRUE ~ NA_character_
    )
  ) %>%
...
seniority_missing_diag <- df_full %>%
  summarise(
    n_rows = n(),
    n_users = n_distinct(user_id),
    n_missing_seniority_raw = sum(!is.finite(seniority_raw)),
    share_missing_seniority_raw = mean(!is.finite(seniority_raw)),
    n_missing_seniority_bin = sum(is.na(seniority_bin)),
    share_missing_seniority_bin = mean(is.na(seniority_bin)),
    mean_abs_round_change = mean(seniority_round_abs_change, na.rm = TRUE),
    p95_abs_round_change = as.numeric(quantile(seniority_round_abs_change, probs = 0.95, na.rm = TRUE, type = 7)),
    n_outside_1_7_after_rounding = sum(is.finite(seniority_rounded) & (seniority_rounded < 1 | seniority_rounded > 7), na.rm = TRUE)
  )

ts_msg("Seniority missingness and rounding diagnostics:")
print(seniority_missing_diag)

write_csv(seniority_missing_diag, file.path(dirs$diagnostics, "seniority_missingness_rounding_diagnostics.csv"))

seniority_bin_diag <- df_full %>%
  count(seniority_bin, name = "n_rows") %>%
  mutate(share_rows = n_rows / sum(n_rows))

write_csv(seniority_bin_diag, file.path(dirs$diagnostics, "seniority_bin_counts_full_loaded_data.csv"))
ts_msg("Seniority bin counts:")
print(seniority_bin_diag)
...
inventor_ref <- df_full %>%
  ...
  summarise(
    ...
    avg_seniority_career = mean(seniority_raw, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    avg_pat_career = ifelse(is.nan(avg_pat_career), NA_real_, avg_pat_career),
    avg_seniority_career = ifelse(is.nan(avg_seniority_career), NA_real_, avg_seniority_career),
...
write_csv(inventor_ref, file.path(dirs$diagnostics, "inventor_level_variables_seniority.csv"))
...
for (window_tag in selected_windows) {
  ...
  run_seniority_descriptives(df_window, window_tag)
...
if (nrow(profile_all) > 0) {
  out_profile <- file.path(dirs$chunk_tables, paste0(chunk_prefix, "__seniority_valid_lifecycle_profiles_all.csv"))
  ...
  ts_msg("Saved seniority-valid lifecycle profile table:", out_profile)
...
write_csv(
  tibble(
    script = "ppml_tenure_ladder_seniority.R",

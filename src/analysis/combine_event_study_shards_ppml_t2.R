#!/usr/bin/env Rscript
###############################################################################
# combine_event_study_shards_v2.R
# Combine PPML Δ-scaled event-study coefficients (t2 sample) across shards and plot results
...
OUT_DIR <- file.path(IN_DIR, "aggregated")
...
combine_and_plot <- function(mover_type) {
  cat("\n[INFO] Combining PPML coefficients for mover type:", mover_type, "\n")

  # --- Load all shards once ---
  files <- list.files(IN_DIR, pattern = paste0("coefficients_", mover_type, "_shard.*\\.csv$"), full.names = TRUE)
  if (length(files) == 0L) {
    cat("[WARN] No files found for mover type:", mover_type, "\n")
    return(NULL)
  }
  ...
  aggregate_and_plot <- function(sub_df, color, out_suffix, ylab, title_prefix) {
    ...
    # --- Save combined CSV
    out_csv <- file.path(OUT_DIR, paste0("combined_", mover_type, "_ppml_", out_suffix, "_coefficients.csv"))
    ...
    p <- ggplot(agg_df, aes(x = as.numeric(event_time), y = estimate)) +
      ...
      labs(
        title = paste0(title_prefix, " (PPML, ±2 Sample): ", toupper(mover_type), " Movers"),
    ...
    out_png <- file.path(OUT_DIR, paste0("event_study_", mover_type, "_ppml_", out_suffix, ".png"))
...
cat("\n[INFO] All done ✅ PPML (t2) plots and CSVs saved in:", OUT_DIR, "\n")

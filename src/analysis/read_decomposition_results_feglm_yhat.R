#!/usr/bin/env Rscript
###############################################################################
# read_decomposition_results_feglm_yhat.R
...
for (v in versions) {
  ...
  # Aggregation
  # ----------------------------
  cat("[INFO] Aggregating to state-year level using predicted y_hat...\n")

  agg_state <- decomp %>%
    group_by(first_state, year) %>%
    summarise(
      mean_yhat = mean(y_hat, na.rm = TRUE),
      E_user = mean(fe_user_id, na.rm = TRUE),
      E_firm = mean(fe_first_rcid, na.rm = TRUE),
      E_state = mean(fe_first_state, na.rm = TRUE),
    ...
    filter(mean_yhat > 0) %>%
    mutate(log_E_yhat = log(mean_yhat))
  ...
  if (grepl("covariates", v) && has_tenure) {
    ...
    reg_firm <- lm(E_firm ~ log_E_yhat + mean_tenure + mean_tenure_sq, data = agg_state)
    reg_state <- lm(E_state ~ log_E_yhat + mean_tenure + mean_tenure_sq, data = agg_state)
    ...
    reg_firm <- lm(E_firm ~ log_E_yhat, data = agg_state)
    reg_state <- lm(E_state ~ log_E_yhat, data = agg_state)
    reg_year <- lm(E_year ~ log_E_yhat, data = agg_state)
  }

  alpha_user <- coef(reg_user)["log_E_yhat"]
  alpha_firm <- coef(reg_firm)["log_E_yhat"]
  alpha_state <- coef(reg_state)["log_E_yhat"]
  alpha_year <- coef(reg_year)["log_E_yhat"]

  alpha_results <- data.frame(
    component = c("Inventor_FE", "Firm_FE", "State_FE", "Year_FE"),
    alpha = c(alpha_user, alpha_firm, alpha_state, alpha_year)
  )
  write_csv(alpha_results, REG_RESULTS)
  cat("[INFO] α-decomposition results saved to:", REG_RESULTS, "\n")
  ...
  state_cov_data <- agg_state %>%
    left_join(state_inventors, by = c("first_state", "year")) %>%
    mutate(Total_FE = E_user + E_firm + E_state)
  ...
  if (grepl("covariates", v) && has_tenure) {
    ...
    cov_reg_firm  <- lm(E_firm ~ X_c + mean_tenure + mean_tenure_sq, data = state_cov_data)
    cov_reg_state  <- lm(E_state ~ X_c + mean_tenure + mean_tenure_sq, data = state_cov_data)
  } else {
    ...
    cov_reg_firm  <- lm(E_firm ~ X_c, data = state_cov_data)
    cov_reg_state  <- lm(E_state ~ X_c, data = state_cov_data)
  }

  beta_total <- coef(cov_reg_total)["X_c"]
  beta_user  <- coef(cov_reg_user)["X_c"]
  beta_firm  <- coef(cov_reg_firm)["X_c"]
  beta_state  <- coef(cov_reg_state)["X_c"]

  beta_results <- data.frame(
    component = c("Total_FE", "Inventor_FE", "Firm_FE", "State_FE"),
    beta = c(beta_total, beta_user, beta_firm, beta_state)
  )
  write_csv(beta_results, COVAR_RESULTS)
  cat("[INFO] β-decomposition results saved to:", COVAR_RESULTS, "\n")
  # ----------------------------

...
# from FE outputs of PPML with covariates
# ===================================================
...
packages <- c("readr", "dplyr", "arrow", "ggplot2")
...
# (Optional) If you included first_university in PPML:
...
if (has_univ) {
  decomp <- decomp %>% left_join(fe_univ, by = "first_university")
}
...
# ============================
# Scatter plots (covariate visualization)
# ============================
cat("[INFO] Generating scatter plots from decomposition data...\n")


PLOT_DIR <- file.path(OUT_DIR, "plots_covariates")
dir.create(PLOT_DIR, showWarnings = FALSE, recursive = TRUE)

# --- Merge covariate info (education or position start date) ---
cat("[INFO] Loading covariates for plotting...\n")

df_cov <- open_dataset(DATA_PATH, format = "parquet") %>%
  select(user_id, n_patents, year, first_city, first_startdate_edu) %>%  # soon add first_startdate_pos here
  collect() %>%
  mutate(
...
gA <- ggplot(bin_stats, aes(x = age_mid, y = y_mean)) +
  ...
  labs(
    title = "Binned Scatter: Patents vs Years Since Education",
...
gB <- ggplot(cityyr, aes(x = x_mean, y = y_mean, size = n_inventors)) +
  ...
  labs(
    title = "City-Year Weighted Scatter (≥50 inventors)",

#!/usr/bin/env Rscript
###############################################################################
# EVENT STUDY: MOVES TO MORE GENDER-IMBALANCED CLUSTERS
#
# Design:
#   - Event = inventor's first move across chosen cluster definition
#   - Treatment intensity of interest =
#         destination gender ratio(t-1) - origin gender ratio(t-1)
#   - Gender ratio = share male in cluster-year, built from:
#         GENDER_VAR <- "au_sex_predicted"
#   - Gender ratio computed on FULL SAMPLE (no US filter)
#   - US filter applied RIGHT BEFORE regression
#   - Samples: ALL / MALE / FEMALE
#   - Cluster definitions: PARENT / METRO_PARENT
#   - Weighting: unweighted / weighted by current cluster-year size
#
# Two versions:
#   (1) NO_HORSE_RACE:
#       n_patents ~ event dummies + event dummies x delta_gender_ratio
...
#
# Notes:
#   - Origin gender ratio at t-1 is inventor-specific LOO
#   - Destination gender ratio at t-1 is overall cluster-year value
...
OUT_DIR   <- "/home/epiga/revelio_labs/output/eventstudy_movers_gender_parent_metroparent"
...
# =========================
# FULL-SAMPLE gender-ratio tables
# gender ratio = male share
# =========================
build_gender_ratio_tables <- function(df, cluster_var) {
  dt <- as.data.table(df[, c("user_id", "year", cluster_var, "male"), drop = FALSE])
  setnames(dt, cluster_var, "cluster_id")
  ...
  dt <- unique(
    dt[!is.na(user_id) & !is.na(year) & !is.na(cluster_id) & !is.na(male),
       .(user_id, year, cluster_id, male)]
  )

  if (nrow(dt) == 0) {
    return(list(
      overall = data.table(cluster_id = character(), year = integer(), gender_ratio_value = numeric()),
      loo = data.table(user_id = character(), cluster_id = character(), year = integer(), loo_gender_ratio_value = numeric())
    ))
  }
  ...
  agg[, gender_ratio_value := male_sum / n_gender_obs]
  overall <- agg[, .(cluster_id, year, gender_ratio_value)]

  loo <- merge(dt, agg, by = c("cluster_id", "year"), all.x = TRUE)
  loo[, loo_gender_ratio_value := fifelse(
    n_gender_obs <= 1,
    NA_real_,
    (male_sum - male) / (n_gender_obs - 1)
  )]
  loo <- loo[, .(user_id, cluster_id, year, loo_gender_ratio_value)]
...
# =========================
attach_move_deltas <- function(dt, gender_tables, size_tables, cluster_var) {
  x <- copy(as.data.table(dt))

  movers <- unique(x[!is.na(move_year), .(user_id, move_year, origin_cluster, dest_cluster)])
  if (nrow(movers) == 0) {
    x[, `:=`(
      delta_gender_ratio = 0,
      delta_log_size = 0
    )]
    return(x[])
  }
  ...
  origin_gender <- merge(origin_q, gender_tables$loo, by = c("user_id", "year", "cluster_id"), all.x = TRUE)
  setnames(origin_gender, "loo_gender_ratio_value", "origin_gender_pre")
  ...
  dest_gender <- merge(dest_q, gender_tables$overall, by = c("year", "cluster_id"), all.x = TRUE)
  setnames(dest_gender, "gender_ratio_value", "dest_gender_pre")
  ...
  mv <- merge(
    origin_gender[, .(user_id, origin_gender_pre)],
    dest_gender[, .(user_id, dest_gender_pre)],
    by = "user_id", all = TRUE
  )
  ...
  mv[, delta_gender_ratio := dest_gender_pre - origin_gender_pre]
  mv[, delta_log_size     := dest_log_size_pre - origin_log_size_pre]

  x <- merge(
    x,
    mv[, .(user_id, delta_gender_ratio, delta_log_size)],
    by = "user_id",
    all.x = TRUE
  )
...
# =========================
# Sample splits
# ALL = only inventors with non-missing predicted gender
# =========================
apply_sample_split <- function(df, sample_tag) {
  if (sample_tag == "ALL") {
    df %>% filter(!is.na(male))
  } else if (sample_tag == "MALE") {
    df %>% filter(male == 1L)
  } else if (sample_tag == "FEMALE") {
    df %>% filter(male == 0L)
  } else {
...
extract_event_coefs <- function(model, meta) {
  ...
  out <- tibble(
    ...
    mutate(
      ...
      component = case_when(
        str_detect(term, "_delta_gender$") ~ "Delta_gender_ratio",
    ...
    mutate(
      ...
      outcome_measure = "gender_ratio_male_share",
      sample = meta$sample,
      spec = meta$spec,
      weight = meta$weight,
      n_obs = meta$n_obs,
...
make_event_plot <- function(coef_df, meta, component_keep) {
  ...
  ttl <- switch(
    ...
    "Delta_gender_ratio" = "Interaction with destination-origin gender ratio gap",
  ...
  ylab <- switch(
    ...
    "Delta_gender_ratio" = "Beta on event-time × delta gender ratio",
  ...
  ggplot(z, aes(x = event_time, y = estimate)) +
    ...
    labs(
      title = paste0(
        meta$cluster_def, " | GENDER | ", meta$sample,
        " | ", meta$spec, " | ", meta$weight
      ),
      subtitle = ttl,
      x = "Event time (move year = 0)",
      y = ylab,
      caption = paste0(
        "US-only estimation sample; gender ratio computed on full sample; ",
        "gender ratio = male share; ref = ", meta$ref_event_time
      )
    ) +
    theme_eug()
}
...
extract_combined_paths <- function(model, df_used, meta, event_levels, ref_val) {
  ...
  mover_delta <- df_used %>%
    ...
    distinct(user_id, delta_gender_ratio, delta_log_size) %>%
    filter(is.finite(delta_gender_ratio), is.finite(delta_log_size))

  if (nrow(mover_delta) == 0) return(NULL)

  q_gender <- as.numeric(stats::quantile(
    mover_delta$delta_gender_ratio,
  ...
  for (j in seq_along(q_gender)) {
    ...
    for (k in all_k) {
      if (k == ref_val) {
        ...
        coef_names <- c(
          paste0("evt_", sk),
          paste0("evt_", sk, "_delta_gender"),
          paste0("evt_", sk, "_delta_size")
        )

        if (meta$spec == "NO_HORSE_RACE") {
          weights_vec <- c(1, qg, 0)
        } else {
          weights_vec <- c(1, qg, med_size)
        }
...
make_combined_plot <- function(path_df, meta) {
  ...
  subtitle_txt <- if (meta$spec == "NO_HORSE_RACE") {
    "Combined path: beta_k + q_gender × theta_gender,k"
  } else {
    paste0(
      "Combined path: beta_k + q_gender × theta_gender,k + med(size gap) × theta_size,k",
  ...
  ggplot(path_df, aes(
    ...
    group = gender_gap_quantile, color = gender_gap_quantile, fill = gender_gap_quantile
  )) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    geom_vline(xintercept = -0.5, linetype = "dotted") +
    ...
    labs(
      title = paste0(meta$cluster_def, " | GENDER | ", meta$sample, " | ", meta$spec, " | ", meta$weight),
      subtitle = subtitle_txt,
      x = "Event time (move year = 0)",
      y = "Estimated move effect for chosen gender-ratio gap",
      color = "Delta gender ratio",
      fill = "Delta gender ratio",
      caption = paste0(
        "Curves shown at p25 / p50 / p75 of destination-origin gender ratio gap; ",
        "gender ratio = male share; ref = ", meta$ref_event_time
      )
    ) +
    theme_eug()
...
run_one_event_reg <- function(df, meta, weight_var = NULL) {
  ...
  gender_terms <- paste0(main_terms, "_delta_gender")
  ...
  # Gender interaction
  g2 <- make_event_plot(coef_df, meta, "Delta_gender_ratio")
  if (!is.null(g2)) {
    ggsave(
      filename = file.path(DIRS$plots, paste0("plot_delta_gender__", out_stub, ".png")),
      plot = g2, width = 10, height = 7, dpi = 300
    )
  }

  # Size interaction only in horse-race spec
  if (meta$spec == "HORSE_RACE") {
...
need_cols(ds_names, c(
  ...
  GENDER_VAR, PARENT_VAR, METRO_VAR
))
...
for (clab in names(cluster_defs)) {
  ...
  ts_msg("Building cluster-year gender-ratio tables for:", clab)
  gender_store[[clab]] <- build_gender_ratio_tables(df_full, cvar)
}
...
for (clab in names(cluster_defs)) {
  ...
  ts_msg("Attaching mover-specific gender and size deltas for:", clab)
  dt_gender <- attach_move_deltas(base_dt, gender_store[[clab]], size_store[[clab]], cvar) %>%
    as_tibble()
  ...
  for (sample_tag in c("ALL", "MALE", "FEMALE")) {
    reg_s <- apply_sample_split(reg_us, sample_tag) %>%
      ...
      filter(
        is.na(move_year) |
          (is.finite(delta_gender_ratio) & is.finite(delta_log_size))
      )
...
if (length(all_results) > 0) {
  ...
  write_csv(final_tab, file.path(OUT_DIR, paste0("ALL_EVENTSTUDY_COEFFICIENTS__gender__", SUFFIX, ".csv")))
}

...
df <- open_dataset(INPUT, format = "parquet") %>%
  select(user_id, n_patents, first_rcid, first_state, first_state, year,
         first_startdate_edu, first_startdate_pos, first_country) %>%
  collect()
...
df <- df %>%
  filter(!is.na(n_patents), !is.na(user_id),
         !is.na(first_rcid), !is.na(first_state),
...
y_hat <- predict(ppml_cov, type = "response")
n_pred <- length(y_hat)
n_obs  <- nrow(df)
cat("[DEBUG] Predictions generated:", n_pred, "for", n_obs, "observations in df\n")

# Retrieve the true estimation sample used by Alpaca
if (!is.null(ppml_cov$model)) {
  df_est <- ppml_cov$model
} else if (!is.null(ppml_cov$data)) {
  df_est <- ppml_cov$data
} else {
...
# Harmonize key types before merging (fix for integer64 vs factor mismatch)
...
# Step 2: Keep relevant subset *after* merging predictions
df_base <- df %>%
  select(user_id, first_rcid, first_state, year, n_patents, y_hat)

# ============================
# Read and merge Fixed Effects
# ============================
...
# Harmonize types for safe joining
df_base <- df_base %>%
  mutate(
    user_id = as.character(user_id),
    first_rcid = as.character(first_rcid),
...
# Step 4: Save unified decomposition dataset (including y_hat)

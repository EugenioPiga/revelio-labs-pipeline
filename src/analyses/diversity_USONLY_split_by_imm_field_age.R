...
# Run regressions:
#   (A) Split by immigrant vs native (as before)
#   (B) ALSO run the same specifications on the FULL sample (no split)
#
# Diversity types: origin/job/edu + field (career_modal_class) + tenure-bin ("age")
#
# NEW (requested, preserved):
#   (i) Add specs where immigrant share enters BY ITSELF (no diversity regressor):
#         n_patents ~ immig_share_10y (+ immig_share_sq_10y optional)
#                   + cluster_size_10y + tenure + tenure_sq
#       run for NATIVE, IMMIGRANT, and ALL samples; PPML and OLS;
#       with inventor FE and without inventor FE.
#   (ii) Apply weights = total cluster size (cluster_size_10y) in EVERY regression,
#       including existing diversity+immshare specs and baseline FE regression.
#
...
OUT_BASE  <- "/home/epiga/revelio_labs/output/diversity_USONLY_splitImm_field_age"  # same as before
...
# "Age diversity" via tenure bins
TENURE_BIN_WIDTH <- 5L
TENURE_BIN_MAX   <- 50L

# Field variable choice (pick one)
FIELD_VAR <- "career_modal_class"
...
make_timeinvariant_measures_USONLY <- function(df_us) {
  ...
  field_y  <- cluster_year_div_any(df10, "cluster_id", FIELD_VAR, "field", MIN_CLUSTER) %>%
    select(cluster_id, year, field_div)

  age_y    <- cluster_year_div_any(df10, "cluster_id", "tenure_bin", "tenurebin", MIN_CLUSTER) %>%
    select(cluster_id, year, tenurebin_div)

  stats_y <- origin_y %>%
    left_join(job_y, by = c("cluster_id","year")) %>%
    left_join(edu_y, by = c("cluster_id","year")) %>%
    left_join(field_y, by = c("cluster_id","year")) %>%
  ...
  stats_c <- stats_y %>%
    ...
    summarise(
      ...
      field_div_10y      = mean(field_div, na.rm = TRUE),
...
df_base <- dbGetQuery(con, sql) %>%
  mutate(
    ...
    last_parent_rcid = as.character(last_parent_rcid),
    career_modal_class = str_trim(as.character(career_modal_class)),
    modal_class_y   = str_trim(as.character(modal_class_y))
  )
...
# Immigrant share by itself (no HHI/diversity in RHS) — NATIVE, IMMIGRANT, ALL
...
# =========================
# 8) Baseline cluster FE (US-only) — weighted
# =========================

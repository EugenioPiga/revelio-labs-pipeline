#!/usr/bin/env Rscript
###############################################################################
# ppml_patents_tenure_top10_nofirmFE_feglm.R
...
OUT_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_state.rds")
SUMMARY_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_state_summary.txt")
...
df <- open_dataset(INPUT, format = "parquet") %>%
  select(user_id, n_patents, first_state, year, 
         first_startdate_edu, first_startdate_pos, first_state, first_country) %>%
  collect()

# only keep US inventors
df <- df %>% filter(first_country == "United States")
...
ppml_top10_nofirmFE <- alpaca::feglm(
  formula = n_patents ~ tenure + tenure_sq | user_id + first_state + year,
  data = df,
...
print(summary(ppml_top10_nofirmFE))
cat("===============================================================\n\n")

saveRDS(ppml_top10_nofirmFE, OUT_FILE)
sink(SUMMARY_FILE)
...
FE_DIR <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_state_fe")
dir.create(FE_DIR, showWarnings = FALSE, recursive = TRUE)

fe_list <- alpaca::getFEs(ppml_top10_nofirmFE)
...
y_hat <- predict(ppml_top10_nofirmFE, type = "response")
n_pred <- length(y_hat)
n_obs  <- nrow(df)
...
if (!is.null(ppml_top10_nofirmFE$model)) {
  df_est <- ppml_top10_nofirmFE$model
} else if (!is.null(ppml_top10_nofirmFE$data)) {
  df_est <- ppml_top10_nofirmFE$data
} else {
...
df <- df %>%
  left_join(
    df_est %>% select(user_id, first_state, year, y_hat, tenure, tenure_sq),
    by = c("user_id", "first_state", "year")
  )

cat("[INFO] Merged predictions back into full dataset.\n")
cat("[DEBUG] Non-missing y_hat:", sum(!is.na(df$y_hat)), "of", nrow(df), "\n")

# Ensure tenure variables exist
df <- df %>%
  mutate(year = as.numeric(year))

if (!("tenure" %in% names(df))) {
  cat("[WARN] tenure variable not found in df — re-computing.\n")
  df <- df %>%
    mutate(
      edu_start_year = as.numeric(substr(first_startdate_edu, 1, 4)),
      pos_start_year = as.numeric(substr(first_startdate_pos, 1, 4)),
  ...
  cat("[INFO] tenure variables found — keeping existing columns.\n")
}

# Step 2: Keep the relevant subset
df_base <- df %>%
  select(user_id, first_state, year, n_patents, y_hat, tenure, tenure_sq)
...
out_decomp <- file.path(OUT_DIR, "decomposition_joined_covariates_feglm_top10_nofirmFE_state.csv")
...
INPUT <- "/labs/khanna/linkedin_202507/processed/inventor_year_merged"
OUT_DIR <- "/home/epiga/revelio_labs/output/regressions"
OUT_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_city.rds")
SUMMARY_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_city_summary.txt")
...
df <- open_dataset(INPUT, format = "parquet") %>%
  select(user_id, n_patents, first_state, year,
         first_startdate_edu, first_startdate_pos, first_city, first_country) %>%
  collect()
...
ppml_top10_nofirmFE <- alpaca::feglm(
  formula = n_patents ~ tenure + tenure_sq | user_id + first_city + year,
  data = df,
  family = poisson(link = "log"),
  control = alpaca::feglmControl(
    dev.tol = 1e-8,
...
FE_DIR <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_city_fe")
dir.create(FE_DIR, showWarnings = FALSE, recursive = TRUE)
...
df <- df %>%
  left_join(
    df_est %>% select(user_id, first_city, year, y_hat, tenure, tenure_sq),
    by = c("user_id", "first_city", "year")
  )
...
# Step 2: Keep the relevant subset
df_base <- df %>%
  select(user_id, first_city, year, n_patents, y_hat, tenure, tenure_sq)
...
# Step 6: Save the unified decomposition file (includes y_hat)
out_decomp <- file.path(OUT_DIR, "decomposition_joined_covariates_feglm_top10_nofirmFE_city.csv")
write_csv(decomp, out_decomp)
cat("[INFO] Decomposition dataset (with y_hat) saved to:", out_decomp, "\n")
...
INPUT <- "/labs/khanna/linkedin_202507/processed/inventor_year_merged"
OUT_DIR <- "/home/epiga/revelio_labs/output/regressions"
OUT_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_metro_area.rds")
SUMMARY_FILE <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_metro_area_summary.txt")
...
df <- open_dataset(INPUT, format = "parquet") %>%
  select(user_id, n_patents, first_state, year,
         first_startdate_edu, first_startdate_pos, first_metro_area, first_country) %>%
  collect()
...
ppml_top10_nofirmFE <- alpaca::feglm(
  formula = n_patents ~ tenure + tenure_sq | user_id + first_metro_area + year,
  data = df,
  family = poisson(link = "log"),
  control = alpaca::feglmControl(
    dev.tol = 1e-8,
...
FE_DIR <- file.path(OUT_DIR, "ppml_covariates_feglm_top10_nofirmFE_metro_area_fe")
dir.create(FE_DIR, showWarnings = FALSE, recursive = TRUE)
...
df <- df %>%
  left_join(
    df_est %>% select(user_id, first_metro_area, year, y_hat, tenure, tenure_sq),
    by = c("user_id", "first_metro_area", "year")
  )
...
# Step 2: Keep the relevant subset
df_base <- df %>%
  select(user_id, first_metro_area, year, n_patents, y_hat, tenure, tenure_sq)
...
# Step 6: Save the unified decomposition file (includes y_hat)
out_decomp <- file.path(OUT_DIR, "decomposition_joined_covariates_feglm_top10_nofirmFE_metro_area.csv")
write_csv(decomp, out_decomp)
cat("[INFO] Decomposition dataset (with y_hat) saved to:", out_decomp, "\n")
